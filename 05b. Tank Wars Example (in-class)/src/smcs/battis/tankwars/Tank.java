...
public class Tank extends Sprite implements Runnable {
	...
	private void move(int direction) {
		...
		if (overlapping().size() != 0) {
			body.move(Math.cos(heading) * MOVE * direction * -1, Math.sin(heading) * MOVE * direction * -1);
		}
		redraw();
	}
	...
	public BoundingCircle getBoundingCircle() {
		BoundingCircle bounds = new BoundingCircle(body.getX(), body.getY(), body.getWidth(), body.getHeight(), body.getCanvas());
		bounds.hide();
		return bounds;
	}
}
